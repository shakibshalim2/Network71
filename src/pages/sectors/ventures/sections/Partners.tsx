      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid lg:grid-cols-[1fr_1.4fr] gap-14 items-start">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {c.items.map((p, i) => (
              <div key={p.title} className="p-6 rounded-2xl sv-partner" style={{ ['--pa' as string]: INDIGO, ['--i' as string]: i, background: 'var(--fill-1)', border: '1px solid var(--line)' }}>
                <span className="sv-partner__tab font-mono">{String(i + 1).padStart(2, '0')}</span>
                <h3 className="font-semibold text-white text-sm mb-2 mt-3">{p.title}</h3>
                <p className="text-slate-400 text-xs leading-relaxed">{p.desc}</p>
                <span className="sv-partner__arrow" aria-hidden="true">
