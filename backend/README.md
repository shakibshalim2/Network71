...
Configure the private `smtp` array from config/example.php, then run `php backend/bin/send-outbox.php` every minute through cPanel cron. SMTP is disabled by default. The worker sends a responsive Network71-branded HTML notification with an embedded logo, enquiry details, a reply action and a plain-text fallback. It uses authenticated TLS via PHPMailer, a non-overlapping file lock, batches of 20, exponential retry and a maximum of 10 attempts. Failed messages remain recorded with the enquiry. SMTP acceptance followed by a process/database failure can result in duplicate delivery on retry; a stable Message-ID is used, but exactly-once delivery is not claimed.
...
`php backend/tests/email-template.php` verifies HTML escaping, the HTML/plain multipart structure and the embedded logo MIME part. `php backend/tests/outbox.php` simulates provider failure and recovery on one isolated development fixture. Actual external delivery must still be checked on the target cPanel host. The implementation follows the [PHPMailer SMTP documentation](https://github.com/PHPMailer/PHPMailer).
...
## Admin workspace features (September 2026 additions)

- **Inbox & Applications**: status chips with live counts, assignee filter (including *Unassigned*), search across name / email / company / subject / reference, CSV export honouring the active filter (`GET /admin/inquiries/export`, `GET /admin/applications/export`; BOM + formula-injection neutralised; audited). Applications gained internal notes (`POST /admin/applications/{id}/notes`, migration `010_application_notes.sql`). Enquiry emails have a one-click copy and a pre-filled reply subject.
- **Content**: list filter by status (`?status=draft|in_review|approved|published|changes|archived`) and a *Duplicate* action that opens a new draft pre-filled from any record.
